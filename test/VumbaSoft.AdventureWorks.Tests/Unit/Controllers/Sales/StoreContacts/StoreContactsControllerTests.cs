...
{
    public class StoreContactsControllerTests : ControllerTests
    {
        private StoreContactsController controller;
        private IStoreContactValidator validator;
        private IStoreContactService service;
        private StoreContactView contact;

        public StoreContactsControllerTests()
        {
            validator = Substitute.For<IStoreContactValidator>();
            service = Substitute.For<IStoreContactService>();

            contact = ObjectsFactory.CreateStoreContactView();

            controller = Substitute.ForPartsOf<StoreContactsController>(validator, service);
        ...
        {
            service.GetViews().Returns(Array.Empty<StoreContactView>().AsQueryable());
        ...
        {
            service.Get<StoreContactView>(contact.Id).Returns(contact);
