...
{
    public class CountryRegionsControllerTests : ControllerTests
    {
        private CountryRegionsController controller;
        private ICountryRegionValidator validator;
        private ICountryRegionService service;
        private CountryRegionView region;

        public CountryRegionsControllerTests()
        {
            validator = Substitute.For<ICountryRegionValidator>();
            service = Substitute.For<ICountryRegionService>();

            region = ObjectsFactory.CreateCountryRegionView();

            controller = Substitute.ForPartsOf<CountryRegionsController>(validator, service);
        ...
        {
            service.GetViews().Returns(Array.Empty<CountryRegionView>().AsQueryable());
        ...
        {
            service.Get<CountryRegionView>(region.Id).Returns(region);
